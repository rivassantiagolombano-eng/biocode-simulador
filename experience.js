...
(() => {
 'use strict';
 const B=BioScience, el=id=>document.getElementById(id), stages=['simulador','transcripcion','arnm','ribosoma','proteina'];
 ...
 function protein(){const t=B.translate(rna);chain(el('guide-protein'),t.chain);label('protein-title',t.stop?'¡Síntesis de proteína completada!':t.start<0?'No comenzó la traducción':'Un fragmento todavía incompleto');label('protein-message',t.stop?'El ADN dio lugar al ARNm. El ribosoma leyó sus codones, unió '+t.chain.length+' aminoácidos y encontró '+t.rows.at(-1).codon+', una señal STOP.':t.message);}
 function mutationSource(){const strand=el('guide-strand').value;return B.transcribe(dna,strand)===rna?{seq:dna,strand}:{seq:rna.replace(/U/g,'T'),strand:'coding'};}
 function mutationDNA(){return mutationSource().seq;}
 function mutationLetters(){const source=mutationDNA(),node=el('mutation-letters');node.replaceChildren();label('selected-base','Base '+selected+' · '+source[selected-1]);for(let offset=0;offset<source.length;offset+=3){const codon=document.createElement('span');codon.className='letter-triplet';[...source.slice(offset,offset+3)].forEach((base,i)=>{const n=offset+i+1,button=document.createElement('button');button.className='letter-button base-'+base;button.textContent=base;button.setAttribute('aria-label',`Base ${n}: ${base}`);button.setAttribute('aria-pressed',String(n===selected));button.onclick=()=>{selected=n;clearResult();mutationLetters();};codon.append(button);});node.append(codon);}}
 ...
 function mutateGuide(){clearResult();try{
   const originalDNA=mutationDNA(),base=el('guide-base').value;
   const changedDNA=originalDNA.slice(0,selected-1)+base+originalDNA.slice(selected);
   const strand=mutationSource().strand,changedRNA=B.transcribe(changedDNA,strand);
   result=B.mutate(B.transcribe(originalDNA,strand),selected,changedRNA[selected-1]);
   result.originalDNA=originalDNA;result.changedDNA=changedDNA;
   ...
   const flow=document.createElement('div');flow.className='mutation-flow';
   for(const [title,before,after]of [['ADN',group(originalDNA),group(changedDNA)],['ARNm',group(result.original),group(result.changed)],['Codón',old||'Fuera de la región traducida',newCodon||'Fuera de la región traducida'],['Aminoácido',oldName,newName]]){const item=document.createElement('div'),heading=document.createElement('b'),a=document.createElement('span'),b=document.createElement('span');heading.textContent=title;a.textContent=before;b.textContent='→ '+after;item.append(heading,a,b);flow.append(item);}box.prepend(flow);
 ...
 function compareGuide(){if(!result)return;const wrap=el('guide-comparison');wrap.replaceChildren();
  [['Secuencia original',result.original,result.normal,result.mutated],['Secuencia mutada',result.changed,result.mutated,result.normal]].forEach(([title,seq,t,other],n)=>{
   ...
   const dnaLine=document.createElement('p');dnaLine.className='comparison-dna';dnaLine.textContent='ADN: '+group(n?result.changedDNA:result.originalDNA);sequence.setAttribute('aria-label','ARNm');const rnaLabel=document.createElement('b');rnaLabel.textContent='ARNm ↓';const beads=document.createElement('div');beads.className='chain';chain(beads,t.chain,other.chain);const note=document.createElement('p');note.className='hint';note.textContent=t.message;card.append(h,dnaLine,rnaLabel,sequence,beads,note);wrap.append(card);
 ...
 function show(){let target=location.hash.slice(1)||'inicio';if(target==='main')target=view;if(!document.getElementById(target)?.classList.contains('view'))target='inicio';
  ...
  document.querySelectorAll('header nav a').forEach(a=>{const active=a.hash==='#'+target||(stage>=0&&a.hash==='#simulador')||(target==='resultado'&&a.hash==='#mutaciones');a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
  if(target==='transcripcion'){label('guide-dna-output',group(dna));label('guide-rna-output',group(rna));}
  if(target==='arnm')label('messenger-sequence',group(rna));
 ...
 el('dna-continue').onclick=()=>{try{const nextDNA=B.clean(el('guide-dna').value,'DNA'),nextRNA=B.transcribe(nextDNA,el('guide-strand').value);dna=nextDNA;rna=nextRNA;resetGuide();label('guide-dna-status','');location.hash='transcripcion';}catch(e){label('guide-dna-status',e.message);}};
